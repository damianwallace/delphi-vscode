...
const scripts_1 = require("../../src/runner/scripts");
...
describe('Branch 1 — fix/delphi-run-script-bugs', function () {
    ...
    it('Bug 1 regression: $PROJECT does not contain .dproj twice (no double-convert)', function () {
        ...
        assert.ok(!scriptContent.includes('.dprojoj'), 'Double-conversion artefact (.dprojoj) found in script');
    });
    it('Bug 2 fix: $exePath is a fully resolved absolute path in a single-quoted PS string', function () {
        ...
        assert.match(scriptContent, /\$exePath\s*=\s*'[A-Za-z]:\\/, "$exePath must be a resolved Windows absolute path in a PS single-quoted string ('C:\\...')");
    });
    it('Bug 3 fix: Start-Process includes -WorkingDirectory', function () {
        assert.ok(scriptContent.includes('-WorkingDirectory'), 'Script is missing -WorkingDirectory in Start-Process call');
    });
    ...
    it('PS-safe fix: $env:DCC_UnitSearchPath uses single-quoted PS string', function () {
        ...
        assert.match(scriptContent, /\$env:DCC_UnitSearchPath\s*=\s*'/, "$env:DCC_UnitSearchPath must use a single-quoted PS string");
    });
    it('PS-safe fix: $TMLIB is fully resolved, not built from "$TMROOT\\lib"', function () {
        ...
        assert.ok(!scriptContent.includes('"$TMROOT'), 'Found old "$TMROOT..." double-quoted construction in script');
    });
});
// ---------------------------------------------------------------------------
...
describe('dccOptions -U parsing — ArFc-style config', function () {
    ...
    before(async function () {
        ...
        vscode_1.mockState.configFile = fixtureUri;
        await (0, scripts_1.initRunScript)(fixtureUri);
        const applyEditCalls = vscode_1.calls['workspace.applyEdit'];
        assert.ok(applyEditCalls && applyEditCalls.length > 0, 'workspace.applyEdit was never called');
        const edit = applyEditCalls[applyEditCalls.length - 1][0];
        scriptContent = edit.getScriptContent(0);
    });
    it('injects $env:DCC_UnitSearchPath from -U paths, not from file:// browsingPaths URIs', function () {
        ...
        assert.ok(!scriptContent.includes('file%3A') && !scriptContent.includes('file:///'), 'file:// URI-encoded path found in $env:DCC_UnitSearchPath — browsingPaths must not be used');
        // The -U paths from dccOptions must appear (real Windows path).
        assert.ok(scriptContent.includes('C:\\tmwin\\tmwincur\\lib'), 'Expected C:\\tmwin\\tmwincur\\lib from -U dccOptions flag to be in DCC_UnitSearchPath');
    });
    ...
    it('DCC_UnitSearchPath includes quoted -U path (program files with spaces)', function () {
        assert.ok(scriptContent.includes('embarcadero\\studio\\23.0\\lib\\Win32\\debug'), 'Expected quoted path with spaces from -U to be unquoted and included');
    });
});
